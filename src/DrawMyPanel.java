...
public class DrawMyPanel extends JPanel {
  ...
  public void openImageToJPanel() {
    try {
      ...
      System.out.println(SteganographyUtil.decode(image));
  ...
  public void saveImageWithLinkedList() {
    try {
      ...
      if (userSelection == JFileChooser.APPROVE_OPTION) {
        ...
        // Convert myShapes to a String
        StringBuilder data = new StringBuilder();
        for (GcuShape shape : myShapes) {
          data.append(shape.toString()).append(";");
        }

        // Encode the data into the image
        SteganographyUtil.encode(img, data.toString(), filePath);
      }
    } catch (FileAlreadyExistsException e) {
      JOptionPane.showMessageDialog(null, "This file already exists");
    } catch (Exception e) {
